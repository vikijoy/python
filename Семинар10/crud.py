...
def currency(id, cur_name):
    global global_id
    global db
    global db_file_name
    stro = ''
    if id == 1:
        data = requests.get('https://openexchangerates.org/api/currencies.json?app_id=264f301f9c414391b07a9d729d14ce88').json()
        #return data.items()
        for key, value in data.items():
            stro = stro + key + ' - ' + value + "\n"
    elif id == 2:
        data = requests.get(f"https://openexchangerates.org/api/latest.json?app_id=264f301f9c414391b07a9d729d14ce88&base=USD&symbols={cur_name}").json()
        #return data['rates'].items()
        for key, value in data['rates'].items():
            stro = stro + key + ' - ' + str(value) + "\n"
    return stro
    #data = requests.get('https://openexchangerates.org/api/latest.json?app_id=264f301f9c414391b07a9d729d14ce88&base=USD&symbols=RUB').json()
    #print(data['USD_RUB'])
