...
def ls_menu():
    while True:
        ...
        if n == 1:
            ...
            lg.logging.info('The user has selected item number 11')
            print('1. Список кодов валют.')
            print('2. Курс валюты к долару США.')
            change = сhecking_the_number(input('Введите номер пункта: '))

            if change == 1:
                lg.logging.info('The user has selected item number 11.1')
                lg.logging.info('User entered: {user_id}')
                print(cr.currency(id=change, cur_name=''))
                #items = cr.currency(id=change,cur_name='')
                #for key, value in items:
                #    print(key, '-', value)

            elif change == 2:
                lg.logging.info('The user has selected item number 11.2')
                cur = input('Введите коды валют (можно через запятую): ')
                lg.logging.info('User entered: {user_id}')
                print(cr.currency(id=change, cur_name=cur))
                #items = cr.currency(id=change, cur_name=cur)
                #for key, value in items:
                #    print(key, '-', value)
                #print(items)
